...
const DeleteUser = ({ match }) => {
    ...
    const currentUserDelete = list.find(user => user.id == userdel);
    const dispatch = useDispatch();
    const startdata = {
        email: currentUserDelete.email,
        name: currentUserDelete.name,
        photo: currentUserDelete.photo

    }

    if (currentUserDelete) {
        console.log("Current user", currentUserDelete);
    }
    const onDelete = () => {
        try{
